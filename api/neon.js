...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'merchant_rpc') {
      ...
      const business = (await db`SELECT id, name, loyalty_rate, qr_token FROM loyalty_businesses WHERE id=${businessId}`).at(0) || (await db`SELECT id, name, loyalty_rate, qr_token FROM loyalty_businesses ORDER BY id LIMIT 1`).at(0);
