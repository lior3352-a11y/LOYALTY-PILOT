...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'register_business') {
      const name = String(p.name || '').trim(), email = String(p.email || '').trim().toLowerCase(), password = String(p.password || '');
      ...
      const business = (await db`INSERT INTO loyalty_businesses(name,qr_token) VALUES(${name},${randomUUID()}) RETURNING id,name,loyalty_rate,qr_token`).at(0);
    ...
    if (action === 'merchant_rpc') {
      ...
      if (name.includes('business_transactions')) return res.json(await db`SELECT t.id transaction_id,c.name,c.phone,t.purchase_amount,t.earned_amount,t.type,t.created_at FROM loyalty_transactions t JOIN loyalty_customers c ON c.id=t.customer_id AND c.business_id=t.business_id WHERE t.business_id=${businessId} ORDER BY t.created_at DESC LIMIT 100`);
