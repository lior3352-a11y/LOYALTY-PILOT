...
export default async function handler(req, res) {
  ...
  if (!process.env.ISRAEL_DATABASE_URL || process.env.ISRAEL_DATABASE_URL === process.env.DATABASE_URL) {
    return res.status(500).json({ ok: false, database: 'isolated Israel database not configured' });
  }
  try {
